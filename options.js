...
async function save_options(event) {
    ...
    const neitherChecked =
        !currentValues["add-emoji-check-mark"] &&
        !currentValues["add-emoji-crossed-mark"] &&
        !currentValues["use-strikethrough-effect"];
    if (neitherChecked) {
        statusNode.textContent =
            "Either check mark emoji, crossed mark emoji or strikethrough effect should be selected";
...
async function restore_options() {
    ...
    Object.keys(resolvedOptions).forEach(
        (key) => (document.getElementById(key).checked = resolvedOptions[key])
    );
}
